...
const validTitle = function (value, e) {
    ...
    const isEnum = ['Mr', 'Mrs', 'Miss', 'Others']
    if (!isEnum.includes(value.trim())) { return e.push(`{ Invalid Title:- available titles are ('Mr', 'Mrs', 'Miss', 'Others') }`) }
    return true
}
// ------------- validation of user First Name -------------
...
//---------------------- Street -------
const validStreet = function (street, e) {

    if (street == undefined) { return false }
    if (street.trim() == "") { return e.push(`{ Street of Address can not be empty }`) }
    if (street.trim().length < 5) { return e.push(`{ Minimum 5 characters are required for street. }`) }
    if (street.trim().length > 30) { return e.push(`{ maximum 30 characters are allowed for street. }`) }

    let regex1 = /^([a-zA-Z 0-9 .,-/]){5,30}$/
    let validRegex1 = regex1.test(street.trim())
    if (validRegex1 == false) { return e.push(`{ Street of Address address is in wrong format, available characters are ( A-Z a-z 0-9 /.,- ) }`) }

    return true
}

//----------------------- City -------
const validCity = function (city, e) {
    if (city == undefined) { return false }
    if (city.trim() == "") { return e.push(`{ City of Address can not be empty }`) }
    if (city.trim().length < 3) { return e.push(`{ Minimum 3 are required for City of Address }`) }
    if (city.trim().length > 20) { return e.push(`{ maximum 20 chacracters are allowed for City of Address }`) }

    let regex2 = /^([a-zA-Z .,]){3,20}$/
    let validRegex2 = regex2.test(city.trim())
    if (validRegex2 == false) { return e.push(`{ City of Address in wrong format, available characters are ( A-Z a-z ., ) } `) }
    return true
...
//--------------------- Pincode -------
const validPincode = function (pincode, e) {
    if (pincode == undefined) { return false }

    let regex = /^[0-9]{6}/
    let validRegex = regex.test(pincode)
    if (validRegex == false) { return e.push(`{ Pincode of Address must be of 6 digits only }`) }
    return true
}
...
module.exports = {
    validTitle, validFName, validLName, validPhone, validEmail, validPassword, validStreet, validCity, validPincode,
    validAddress, validPW_4_Login, validProfileImage, validRest, V_address, V_Sub_address, V_street, V_city, V_pincode
}